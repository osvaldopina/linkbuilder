...
public class QueryParameterVariableValueDiscoverTest extends EasyMockSupport {
	...
	@TestSubject
	QueryParameterVariableValueDiscover queryParameterVariableValueDiscover =
			new QueryParameterVariableValueDiscover(introspectionUtils);

	@Test
	public void getVariableValues() throws Exception {
		expect(methodCall.getMethod()).andReturn(method);
		expect(introspectionUtils.getQueryVariableName(method, parameterIndex)).andReturn(variableName);
	...
	public void canDiscover() throws Exception {
		...
		assertThat(queryParameterVariableValueDiscover.canDiscover(methodCall, parameterIndex), is(true));

		verifyAll();
	}

	@Test
	public void canDiscover_notAQueryParameter() throws Exception {
		expect(methodCall.getMethod()).andReturn(method);
		expect(introspectionUtils.isQueryVariableParameter(method, parameterIndex)).andReturn(false);

		replayAll();

		assertThat(queryParameterVariableValueDiscover.canDiscover(methodCall, parameterIndex), is(false));
